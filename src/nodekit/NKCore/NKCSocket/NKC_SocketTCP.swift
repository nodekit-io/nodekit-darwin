  ...
  class NKC_SocketTCP: NSObject, NKScriptExport {
    ...
    class func rewriteGeneratedStub(stub: String, forKey: String) -> String {
        switch (forKey) {
        case ".global":
            return NKStorage.getPluginWithStub(stub, "lib-core.nkar/lib-core/platform/tcp.js", NKC_SocketTCP.self)
        default:
            return stub
        }
    }
    ...
    override init() {
        ...
        super.init()
        self._socket!.setDelegate(self, delegateQueue: NKScriptContextFactory.defaultQueue )
    }
  ...
  extension NKC_SocketTCP: NKC_SwiftSocketProtocol {
    func socket(socket: NKC_SwiftSocket, didAcceptNewSocket newSocket: NKC_SwiftSocket) {
        ...
        newSocket.setDelegate(socketConnection, delegateQueue: NKScriptContextFactory.defaultQueue )
